...
test.describe('Inloggning', () => {
  test('ska kunna logga in med giltiga uppgifter', async ({ page }) => {
    ...
    await expect(page.locator('[class*="error"], [class*="bg-red"], .text-red')).toBeVisible({ timeout: 10000 });
  });
  ...
  test('ska visa felmeddelande vid ogiltiga uppgifter', async ({ page }) => {
    ...
    const errorMessage = page.locator('[class*="error"], [class*="bg-red"], .text-red');
    await expect(errorMessage.first()).toBeVisible({ timeout: 5000 });
  });

  test('ska kunna navigera mellan inloggning och registrering', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Registrera' })).toBeVisible();
